...
func (pxCanvas *PxCanvas) Bounds() image.Rectangle {
	...
	x1 := pxCanvas.PxCols*pxCanvas.PxSize + int(pxCanvas.CanvasOffset.X)
	y1 := pxCanvas.PxRows*pxCanvas.PxSize + int(pxCanvas.CanvasOffset.Y)
...
func (pxCanvas *PxCanvas) CreateRenderer() fyne.WidgetRenderer {
	...
	for i := 0; i < len(canvasBorder); i++ {
		canvasBorder[i].StrokeColor = color.NRGBA{R: 100, G: 100, B: 100, A: 255}
...
func (pxCanvas *PxCanvas) SetColor(c color.Color, x, y int) {
	if nrgb, ok := pxCanvas.PixelData.(*image.NRGBA); ok {
		nrgb.Set(x, y, c)
	}
